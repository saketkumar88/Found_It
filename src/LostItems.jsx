...
function LostItems() {
  ...
  const handleClaim = async (item) => {
    const claimerId = localStorage.getItem("collegeId");
    if (!claimerId) return alert("Please log in first!");

    // Simple prompt to get a message for the owner
    const proof = window.prompt("Enter a message for the owner (e.g., 'I found your keys near the canteen'):");
    if (!proof) return;
    ...
    try {
      const response = await fetch("http://localhost:8080/api/claims/request", {
        ...
        body: JSON.stringify({
          itemId: item._id,
          claimerCollegeId: claimerId.toLowerCase(),
          founderCollegeId: item.reportedBy.toLowerCase(),
      ...
      if (response.ok) {
        alert("Owner notified! You will receive Karma once they approve.");
      } else {
  ...
  return (
    <div className="min-h-screen text-white p-4 md:p-8 relative bg-transparent">
          ...
          <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full whitespace-nowrap text-xs font-bold transition-all border ${activeTab === cat ? 'bg-blue-500 border-blue-500 text-black shadow-lg shadow-blue-500/30' : 'bg-white/5 border-white/10 hover:bg-white/10'
                  }`}
              >
                {cat.toUpperCase()}
              </button>
        ...
        {loading ? (
          ...
          displayedItems.map((item) => (
            <div
              ...
              className={`bg-gray-900/40 backdrop-blur-md p-6 rounded-[2.5rem] relative flex flex-col border-2 group transition-all duration-300 ${itemId === item._id
            ...
            >
              ...
              <div className="mt-auto pt-6 border-t border-white/5">
                ...
                {/* Logic: Only show the action button if the current user is NOT the reporter */}
                {item.reportedBy !== currentUser ? (
                  <button
      ...
      ${item.status === 'Resolved'
                  ...
                  >
                    {item.status === 'Resolved' ? "Already Returned ✅" : "Found This 🎯"}
                  </button>
                ) : (
                  <button
                    disabled
                    className="w-full py-4 bg-gray-800 text-gray-400 rounded-2xl font-black uppercase text-xs tracking-widest cursor-not-allowed opacity-50"
                  >
                    Your Post
                  </button>
                )}
