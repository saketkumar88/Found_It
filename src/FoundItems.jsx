    ...
    try {
        const response = await fetch("http://localhost:8080/api/claims/request", {
            ...
            body: JSON.stringify({
                ...
                founderCollegeId: claimingItem.reportedBy,
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full whitespace-nowrap text-xs font-bold transition-all border ${
                  activeTab === cat ? 'bg-cyan-500 border-cyan-500 text-black' : 'bg-white/5 border-white/10 hover:bg-white/10'
                }`}
              >
                {cat.toUpperCase()}
              </button>
        ...
        {loading ? (
          ...
          displayedItems.map((item) => (
            <div 
              ...
              className={`bg-gray-900/40 backdrop-blur-xl p-6 rounded-[2.5rem] relative flex flex-col border-2 group transition-all duration-300 ${
                itemId === item._id ? 'border-cyan-500 shadow-2xl w-full max-w-md' : 'border-white/5 hover:border-cyan-500/30'
              }`}
            >
              ...
              <div className="mt-auto pt-6 border-t border-white/5 space-y-4">
                ...
                {item.reportedBy !== currentUser ? (
                    <button 
                      onClick={() => { setClaimingItem(item); setIsClaimModalOpen(true); }} 
                      className="w-full py-4 bg-cyan-500 text-black rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-white hover:scale-[1.02] transition-all shadow-lg shadow-cyan-500/10"
                    >
                      Claim Now
                    </button>
                ) : (
